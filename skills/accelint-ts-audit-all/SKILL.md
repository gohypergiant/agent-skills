...
### Scope Validation
- **Is this path valid and accessible?** Verify that the file or directory exists before you create TODO lists.
- **Are there test and build commands available?** Check `package.json` or ask the user for verification commands before you start.
- **How many files will this audit?** Large directories (>5 files) usually require multiple sessions. Set expectations up front.

### Session Management
- **Is this a new audit or a resumed audit?** Check for existing audit-process files in `.agents/audit/` before you create new ones.
- **Will this fit in one session?** Estimate about 1-5 files per session at most. Plan for resumption if the audit is larger.
- **Are verification commands known?** Document the exact test/build/lint commands in the audit-process file from the start.

### Change Philosophy
- **What is the user's risk tolerance?** Some users want every suggestion. Others want only critical fixes. Clarify this before the first interactive prompt.
- **Should performance micro-optimizations be included?** Gains in the 1.05x-1.15x range may not justify the code churn for all projects.
...
This skill is command-only. Run it only when it is explicitly invoked with `/skill accelint-ts-audit-all <path>`.
Do not trigger it from natural-language requests unless the user explicitly asks to use this skill.
...
**Note:** Ensure that `.agents/audit/` is in your project's `.gitignore` so audit tracking files are not committed.
...
Before creating anything, capture the original repository root and branch so you can safely return later.
...
**Important:** Create the worktree in `.agents/worktrees/` (not `.agents/audit/`) to avoid conflicts with the gitignored `.agents/audit/` directory where tracking files are stored.
...
- Create `.agents/audit/audit-process-${timestamp}.md` (use the same timestamp as the worktree)
- Create `.agents/audit/audit-history-${timestamp}.md` (use the same timestamp)
...
- `build/**`
- `coverage/**`
- common generated directories such as `.next/**`, `storybook-static/**`, and `vendor/**` when present

If the target path resolves to zero auditable files after those exclusions, stop and tell the user why instead of creating tracking files.
...
If any of those conditions are not met, leave the file in progress and update the resume instructions explicitly.
