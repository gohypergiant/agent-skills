...
test.describe(
	"Basic functionality",
	{
		...
		annotation: {
			type: "source",
			description:
				"agent-skills/skills/accelint-ac-to-playwright/assets/examples/acceptance/sample-app-functionality.feature",
		},
	},
	() => {
		test("user changes circle color from settings", async ({
			page,
		}, testInfo) => {
			...
			try {
				await expect(
					page.getByTestId("page.dropdown.color-choice"),
				).toHaveCount(1);
				await page
					.getByTestId("page.dropdown.color-choice")
					.selectOption("Pink");
			} catch (error) {
				await attachFailureArtifacts({
					page,
					...
					stepIndex: 3,
					action: "select",
					testId: "page.dropdown.color-choice",
				});
				throw error;
			}
		...
		test("user submits name and sees welcome message", async ({
			page,
		}, testInfo) => {
			await page.goto("/");
			...
			try {
				...
				await expect(page.getByTestId("page.text.fname-output")).toContainText(
					"Hello, Harry Potter.",
				);
			} catch (error) {
				await attachFailureArtifacts({
