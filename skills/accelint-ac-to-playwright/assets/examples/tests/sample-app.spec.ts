...
test.describe(
	"Sample app",
	{
		annotation: {
			type: "source",
			description:
				"agent-skills/skills/accelint-ac-to-playwright/assets/examples/acceptance/sample-app.md",
		},
	},
	() => {
		test("navigates to settings from home", async ({ page }, testInfo) => {
			...
			try {
				await expect(page.getByTestId("header.text.page-heading")).toHaveCount(
					1,
				);
				await expect(
					page.getByTestId("header.text.page-heading"),
				...
				await attachFailureArtifacts({
					...
					stepIndex: 3,
					action: "expectText",
					testId: "header.text.page-heading",
				});
				throw error;
			}
		});

		test("navigates to home from settings", async ({ page }, testInfo) => {
