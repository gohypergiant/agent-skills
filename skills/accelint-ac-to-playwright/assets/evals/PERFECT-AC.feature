...
Feature: Comprehensive action and assertion coverage
  ...
  Scenario: Keyboard modifier combination
    ...
    Then the banner text on the header says 'Edit mode activated'
  ...
  Scenario: Mouse drag operation
    ...
    Then the success text on the toast says 'Zoom area has been set'
  ...
  Scenario: Mouse position, press, and release sequence
    ...
    Then the success text on the toast says 'Point marked'
