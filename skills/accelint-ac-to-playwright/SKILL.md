...
- **NEVER use `mouseMove` without a follow-up action** — `mouseMove` positions the cursor but doesn't interact with anything. It should only be used before actions like `mouseClick`, or when AC explicitly requires moving to specific coordinates before other mouse operations.
