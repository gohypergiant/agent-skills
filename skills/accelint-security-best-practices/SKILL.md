...
description: Use for JavaScript or TypeScript security audits, vulnerability reviews, and secure implementation work focused on authentication, authorization, API endpoints, file uploads, outbound fetch or SSRF flows, secrets, dependency risk, user input handling, or pre-deployment hardening. Trigger on requests such as security review, audit for vulnerabilities, secure this code, review auth flow, check file upload security, assess SSRF risk, or harden this API. Best for repo-, service-, or feature-scoped security work that identifies real risks, prioritizes critical and high issues, maps findings to OWASP categories and severity, and either reports findings or applies concrete fixes without drifting into generic non-security cleanup.
license: Apache-2.0
metadata:
  author: accelint
  version: "1.1.2"
