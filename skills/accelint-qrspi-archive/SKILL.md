...
description: Archive one or more OpenSpec changes end to end and complete the required post-archive bookkeeping. Use when the user wants to run /opsx:archive or /opsx:bulk-archive, archive pending changes, cross-link touched specs with additive related updates, patch openspec/specs/INDEX.md, or append rows to openspec/changes/archive/INDEX.md. Prefer this skill for the full archive-plus-bookkeeping workflow, including native archive execution plus follow-on linking and index maintenance. Do not use for proposing a change, implementing a change, pruning stale related links, changing existing archive status values, or corpus-wide archive synthesis and cleanup.
license: Apache-2.0
