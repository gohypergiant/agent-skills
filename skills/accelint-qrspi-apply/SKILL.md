...
description: Implement an existing QRSPI-planned OpenSpec change by orchestrating /opsx:apply, dependency-aware slice execution, living-document updates, and mandatory verification. Use when the user wants to apply or resume a QRSPI change package, execute checkbox-tracked vertical slices from tasks.md, continue from a partial implementation, or run a change whose plan includes dependency levels or a Parallelization Strategy. Prefer this skill over plain /opsx:apply for QRSPI-generated changes that need slice-aware delegation and end-to-end apply-through-verify workflow. Do not use for planning a change, hand-implementing work outside /opsx:apply, archiving completed changes, or generic OpenSpec requests that are not about executing an existing QRSPI task package.
license: Apache-2.0
