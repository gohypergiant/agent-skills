...
## Rule: Avoid Excessive Snapshot Testing
...
// ❌ Snapshot includes implementation details
test('modal content', () => {
  const { container } = render(<Modal />);
  expect(container.querySelector('.modal-wrapper')).toMatchSnapshot();
});
```

**Problems:**
- Snapshots break on trivial changes (whitespace, class reorder)
...
test('shows error message', () => {
  render(<ErrorPage error="Network timeout" />);
...
// ✅ Snapshot only critical structure
test('formats currency correctly', () => {
  render(<PriceDisplay amount={1234.56} currency="USD" />);
  
  const price = screen.getByRole('region', { name: /price/i });
  expect(price.textContent).toMatchInlineSnapshot('"$1,234.56 USD"');
});
```

**When snapshots are OK:**
- Small, stable structures (error messages, formatted output)
- Component libraries with stable API
- Inline snapshots for specific values

---

## Rule: Don't Use wrapper from render()
