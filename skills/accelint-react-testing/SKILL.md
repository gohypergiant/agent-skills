...
description: Use when writing, reviewing, refactoring, or auditing React component tests that use React Testing Library or Testing Library patterns. Trigger on render(), screen, within(), waitFor, waitForElementToBeRemoved, getBy/findBy/queryBy queries, fireEvent, userEvent, jest-dom matchers, @testing-library/react, @testing-library/user-event, or React test files such as .test.tsx and .spec.tsx. Best for query priority, getBy vs findBy vs queryBy, scoped queries, user-centric interactions, async assertions, provider-aware custom render helpers, accessibility-first checks, and RTL anti-patterns. Do not use for non-React unit tests or Playwright end-to-end work unless the question is specifically about React Testing Library behavior.
