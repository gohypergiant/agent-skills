...
description: Use when the user needs the right TanStack Query or React Query shape for an app, especially how server data should be fetched, keyed, cached, hydrated, invalidated, or updated in React or Next.js. Trigger on setup, reviews, audits, migrations, and debugging when TanStack Query is the main server-state layer, including QueryClient defaults, query-key structure, staleTime and gcTime policy, dependent queries, mutations, optimistic updates, stale data, duplicate requests, and refetch churn. Especially relevant in Next.js App Router for prefetching, dehydrate and HydrationBoundary, per-request QueryClient setup, useSuspenseQuery, or aligning server actions and cache tags with client invalidation. Do not use for SWR, plain fetch, Zustand, local UI state, or backend-only caching unless TanStack Query is central.
license: Apache-2.0
metadata:
  author: accelint
  version: "1.4.2"
